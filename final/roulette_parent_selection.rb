...
private
  ...
  def generate_probabilities(population:, without:)
    ...
    normalized_genotypes.each do |normalized_genotype| 
      population_probabilities[normalized_genotype[:genotype]] = normalized_genotype[:normalized_fitness_value].to_f / normalized_fitness_sum
    end
    population_probabilities
  end
